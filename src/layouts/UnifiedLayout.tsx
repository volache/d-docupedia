...
import { SummaryBox, Callout, ExampleBlock, StepItem, TableBlock, FAQAccordion } from '../components/article/ArticleBlocks';
...
export const UnifiedLayout = ({ content }: { content: any }) => {
  ...
  const renderBlocks = (blocks: any[]) => {
    return blocks.map((block: any, n: number) => {
      const i = n;
      switch (block.type) {
        case 'summary':
          return (
            <div key={i} className="max-w-4xl mx-auto">
               <SummaryBox content={block.content} />
            </div>
          );
        case 'text':
          return (
            <div key={i} className="max-w-4xl mx-auto py-4 px-2 sm:px-0">
              <p className="text-slate-700 text-lg leading-relaxed whitespace-pre-wrap">{block.content}</p>
            </div>
          );
        case 'callout':
          return (
            <div key={i} className="max-w-4xl mx-auto py-2">
               <Callout type={block.style || 'info'} title={block.title} content={block.content} />
            </div>
          );
        case 'example':
          return (
            <div key={i} className="max-w-4xl mx-auto py-6">
        ...
        case 'faq':
          return <FAQAccordion key={i} question={block.question} answer={block.answer} />;
        case 'use_case':
          return (
            <div key={i} className="max-w-4xl mx-auto flex items-start gap-3 bg-slate-50 p-4 rounded-xl mb-3">
        ...
        case 'system_step':
          return (
             <div key={i} className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 py-10 items-center border-b border-slate-50">
